...
# Накатывает бандл карточек naturalist.travel на БД (обычно на сервере).
# Перед применением снимает полный бэкап базы — накат идёт одной транзакцией,
# но бэкап нужен, чтобы откатиться уже после успешного COMMIT.

if [[ $# -lt 1 ]]; then
  echo "Usage: $0 path/to/naturalist-draft-import.sql" >&2
  exit 2
fi
...
REMOTE_BACKUP="/tmp/onlyglamps-before-naturalist-${STAMP}.dump"
LOCAL_BACKUP="${BACKUP_DIR}/onlyglamps-before-naturalist-${STAMP}.dump"

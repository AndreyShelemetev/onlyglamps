...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 path/to/vsaunah-draft-import.sql[.gz] [ssh-host] [remote-project-dir]" >&2
  exit 2
fi
...
scp scripts/import-vsaunah-dump.sh "${SSH_HOST}:${REMOTE_SCRIPTS}/import-vsaunah-dump.sh"

echo "Applying dump on server"
ssh "$SSH_HOST" "set -euo pipefail
  cd '$REMOTE_PROJECT'
  chmod +x scripts/import-vsaunah-dump.sh
  if [[ '$remote_dump' == *.gz ]]; then
    gzip -df '$remote_dump'
  fi
  scripts/import-vsaunah-dump.sh '$remote_sql'
"

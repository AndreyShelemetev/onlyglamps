...
# Собирает переносимый бандл карточек naturalist.travel из локальной БД.
# Накат на сервере — scripts/import-naturalist-dump.sh.
#
# Ключ переноса — SourceLinks.SourceUrl. Опубликованные объекты на приёмнике
# не трогаются: их пропускает сам бандл.

DB_SERVICE="${DB_SERVICE:-db}"
SOURCE_NAME="${SOURCE_NAME:-Натуралист}"
OUT="${1:-deploy/imports/naturalist-draft-import-$(date +%Y%m%d-%H%M%S).sql}"
...
cat > "$OUT" <<'SQL'
-- OnlyGlamps naturalist.travel draft import bundle.
-- Generated from a local database. Apply with scripts/import-naturalist-dump.sh.
...
copy_query "og_import_regions" "slug, name, created_at" \
  "SELECT DISTINCT r.\"Slug\", r.\"Name\", r.\"CreatedAt\"
   ...
   WHERE sl.\"SourceName\" = \$\$${SOURCE_NAME}\$\$
   ORDER BY r.\"Slug\""
...
BEGIN
  ...
  FOR rec IN SELECT * FROM og_import_objects ORDER BY source_url LOOP
    ...
    FROM "SourceLinks" sl
    JOIN "GlampingObjects" o ON o."Id" = sl."ObjectId"
    WHERE sl."SourceUrl" = rec.source_url
    ORDER BY o."Id"
    LIMIT 1;
    ...
    -- Slug на приёмнике может быть занят чужим объектом: на сервере уже есть
    -- карточки других источников, и нумерация slug'ов там своя.
    v_slug := rec.slug;
    v_slug_n := 1;
    WHILE EXISTS (
      SELECT 1 FROM "GlampingObjects"
      WHERE "Slug" = v_slug
        AND (v_object_id IS NULL OR "Id" <> v_object_id)
    ) LOOP
      v_slug_n := v_slug_n + 1;
      v_slug := rec.slug || '-' || v_slug_n;
    END LOOP;
    ...
    IF v_object_id IS NULL THEN
      ...
      VALUES (
        ...
        rec.name, v_slug, rec.short_description, rec.full_description,

...
OUT="${1:-deploy/imports/vsaunah-draft-import-$(date +%Y%m%d-%H%M%S).sql}"
...
cat > "$OUT" <<'SQL'
-- OnlyGlamps vsaunah draft import bundle.
-- Generated from a local database. Apply with scripts/import-vsaunah-dump.sh.
...
copy_query "og_import_objects" "source_url, source_name, source_type, object_type_slug, region_slug, city_slug, name, slug, short_description, full_description, area, capacity, beds, rooms, is_whole, min_rental_days, max_rental_days, address, settlement, latitude, longitude, check_in_time, check_out_time, children_allowed, pets_allowed, smoking_allowed, events_allowed, deposit, rules, moderation_comment, seo_title, seo_description, created_at, updated_at" \
  'SELECT sl."SourceUrl", sl."SourceName", sl."SourceType",
          ...
          o."Deposit", o."Rules", o."ModerationComment", o."SeoTitle", o."SeoDescription",
  ...
  FOR rec IN SELECT * FROM og_import_objects ORDER BY source_url LOOP
    ...
    IF v_object_id IS NULL THEN
      INSERT INTO "GlampingObjects" (
        ...
        "Deposit", "Rules", "ModerationComment", "Status", "SeoTitle", "SeoDescription",
        "CreatedAt", "UpdatedAt"
      )
      VALUES (
        ...
        rec.deposit, rec.rules, rec.moderation_comment, 'Draft', rec.seo_title, rec.seo_description,
    ...
    ELSE
      ...
      SET
        ...
        "Deposit" = rec.deposit,
        "Rules" = rec.rules,
        "ModerationComment" = rec.moderation_comment,

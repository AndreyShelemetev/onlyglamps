...
OUT="${1:-deploy/imports/mirturbaz-draft-import-$(date +%Y%m%d-%H%M%S).sql}"
...
cat > "$OUT" <<'SQL'
-- OnlyGlamps mirturbaz draft import bundle.
-- Generated from a local database. Apply with scripts/import-mirturbaz-dump.sh.
...
copy_query "og_import_regions" "slug, name, created_at" \
  'SELECT DISTINCT r."Slug", r."Name", r."CreatedAt"
   ...
   WHERE sl."SourceName" = $$Мир Турбаз$$
   ORDER BY r."Slug"'
